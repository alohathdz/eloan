...
app.put('/pay/update/:id', (req, res) => {
    db.query("UPDATE loan SET balance = balance + ?, due_date = DATE_ADD(due_date, INTERVAL -1 MONTH) WHERE loan_id = ?", [req.body.old_loan, req.body.loan_id], (err, result) => {
        if (err) {
            ...
            db.query("UPDATE payment SET loan = ?, interest = ?, pay_date = ? WHERE payment_id = ?", [req.body.loan, req.body.interest, pay_date, req.params.id], (err, result) => {
                if (err) {
                    console.log(err);
                } else {
                    db.query("UPDATE loan SET balance = balance - ?, due_date = DATE_ADD(due_date, INTERVAL 1 MONTH) WHERE loan_id = ?", [req.body.loan, req.body.loan_id], (err, result) => {
                        if (err) console.log(err);
                        return res.json(result);
                    });
                }
